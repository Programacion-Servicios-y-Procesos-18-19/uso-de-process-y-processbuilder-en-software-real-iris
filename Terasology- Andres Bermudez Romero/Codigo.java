...
public final class NonNativeJVMDetector {
    ...
    private static boolean posixSystemIs64() throws IOException, InterruptedException {
        Process unameProc = new ProcessBuilder("uname", "-m").start();//aqui se crea el ProcessBuilder
        unameProc.waitFor();
        try (BufferedReader unameStdout = new BufferedReader(new InputStreamReader(unameProc.getInputStream()))) {//Lee lo que se le envia y lo devuelve en el return
